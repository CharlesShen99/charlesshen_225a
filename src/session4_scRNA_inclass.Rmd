...
dittoDimPlot(Tcells, "RNA_snn_res.0.1", size = 1.2, "umap")
```
...
#Tcells$Tage == tage[1]
highlight_cluster <- function(selected_tage, selected_cols){
  dittoDimPlot(Tcells, "Tage", size = 1, reduction.use = "umap",
               cells.use = Tcells$Tage == selected_tage, colors = selected_cols,
...
for (x in 1:6){
  single_plot = highlight_cluster(selected_tage = tage[x], selected_cols = cols[x])
...
# use three colons to use hidden function
